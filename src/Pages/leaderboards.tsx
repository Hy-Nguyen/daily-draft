...
export default function Leaderboards() {
	...
	const currentListing = leaderboardData.slice(
		indexOfFirstPlayer,
		indexOfLastPlayer
	);

	const totalPages = Math.ceil(
		leaderboardData.length / playersPerPage
	);

	// const sortedByPlayed = leaderboardData.sort(
	// 	(a, b) => {
			...
			<main className='bg-lobby flex min-h-screen w-screen justify-center text-[#5A5C6F]'>
				...
				>
					...
					<AnimatePresence>
						<div
							id='my-drafts'
							className='h-full w-3/4 rounded-[8px] bg-[#191A22] p-6'
						>
							<div
								id='header'
							...
							<motion.div
								id='table'
								className='relative mt-6 pb-[70px]'
							>
								<motion.div
									id='header'
